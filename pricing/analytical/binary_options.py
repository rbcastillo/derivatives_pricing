...
class BinaryOption(FinancialProduct):
    """
    Generic abstract implementation of the Binary option object populated with the key auxiliary methods
    ...
    def __init__(self, s: Union[float, int], k: Union[float, int], r: float, t: Union[float, int], sigma: float,
                 q: float = 0) -> None:
        """
        Method to initialize the Binary option class with its relevant input. Please, note that the metrics
    ...
    def get_d1(self) -> float:
        """
        Method to calculate the auxiliary measure d1 used in the Black-Scholes formula for Binary options. The value
    ...
    def price(self) -> float:
        """
        Method to calculate the Binary option price, the implementation needs to be adjusted for either Call or
        Put options in the specific classes.

        :return: Price calculated for the Binary option.
        """
        pass

    def get_gamma(self) -> float:
        """
        The gamma of a Binary option represents the expected change in the delta of the option as a response to
        ...
        {\\partial S^2}`). The gamma of the Binary option is calculated as:
        ...
        :return: Gamma calculated for the Binary option.
        """
        d1 = self.get_d1()
    ...
    def get_vega(self, scale: float = 1.) -> float:
        """
        The vega of a Binary option represents the expected change in the price of the option as a response to
        changes in the volatility of the underlying (:math:`\\small \\displaystyle \\frac{\\partial V}
        {\\partial \\sigma}`). The vega of the Binary option is calculated as:
        ...
        As presented above, :math:`Vega` measures the decrease in value in the Binary option as volatility changes
        a full 100%. Usually this measure is too large to be useful in usual Binary options, where the underlying
        ...
        :return: Vega calculated for the Binary option.
        """
        d1 = self.get_d1()
    ...
    def calc_implied_vol(self, price: Union[int, float], disp: bool = False) -> float:
        """
        The implied volatility of a Binary option is the value for the volatility of the underlying asset that
        ...
        the analytics expression for the value of the Binary option using numerical methods. In this implementation
        the scipy version of the Nelder-Mead simplex algorithm is used.

        :param price: price for the Binary option observed in the market.
        :param disp: set to True to print convergence messages.
        :return: implied volatility for the given market price of the Binary option.
        """
...
class BinaryCall(BinaryOption):
    """
    Implementation of the Binary Call option valuation by using the test_analytical expression. The Binary Call
    option pays the holder $1 if the underlying asset is above the strike price on a specified expiration date
    and zero otherwise.
    """

    def price(self) -> float:
        """
        The price of the Binary Call option is calculated as:

        :math:`\\small \\displaystyle C = e^{-r t}N(d_2)`

        Where :math:`N(x)` represents the CDF of the standard Gaussian distribution and :math:`d_2` is defined in
        its own method.

        :return: Price calculated for the Binary Call option.
        """
        d2 = self.get_d2()
    ...
    def get_delta(self) -> float:
        """
        The delta of a Binary option represents the expected change in the price of the option as a response to
        small changes in the price of the underlying (:math:`\\small \\displaystyle \\frac{\\partial V}{\\partial S}`).
        The delta of the Binary Call option is calculated as:

        :math:`\\small \\displaystyle \\Delta = \\frac{e^{-r t}N'(d_2)}{\\sigma S \\sqrt{t}}`

        Where :math:`N(x)` represents the CDF of the standard Gaussian distribution and :math:`d_1` is defined in
        its own method.

        :return: Delta calculated for the Binary Call option.
        """
        d2 = self.get_d2()
        delta = (exp(-self.r * self.t) * norm.pdf(d2)) / (self.sigma * self.s * sqrt(self.t))
        return delta

    def get_theta(self, periods: int = 1) -> float:
        """
        The theta of a Binary option represents the expected change in the price of the option as a response to
        time passing (:math:`\\small \\displaystyle \\frac{\\partial V}{\\partial t}`). The theta of the Binary Call
        ...
        As presented above, :math:`\\Theta` measures the decrease in value in the Binary option as one full t period
        passes by. Usually t is measured in years, so this measure is too large to be useful in usual Binary options
        ...
        :return: Theta calculated for the Binary Call option.
        """
        d1 = self.get_d1()
        d2 = self.get_d2()
    ...
    def get_rho(self, target: str = 'r', scale: float = 1.) -> float:
        """
        The rho of a Binary option represents the expected change in the price of the option as a response to
        changes in the risk-free interest rate (:math:`\\small \\displaystyle \\frac{\\partial V} {\\partial r}`).
        The rho of the Binary Call option is calculated as:
        ...
        As presented above, :math:`\\rho` measures the decrease in value in the Binary option as the interest rate
        or the dividend yield change a full 100%. Usually this measure is too large to be useful in usual Binary
        ...
        :return: rho calculated for the Binary Call option.
        """
...
class BinaryPut(BinaryOption):
    """
    Implementation of the Binary Put option valuation by using the test_analytical expression. The Binary Put
    option pays the holder $1 if the underlying asset is below the strike price on a specified expiration date
    and zero otherwise.
    """

    def price(self) -> float:
        """
        The price of the Binary Put option is calculated as:

        :math:`\\small \\displaystyle P = e^{-r t}(1 - N(-d_2))`

        Where :math:`N(x)` represents the CDF of the standard Gaussian distribution and :math:`d_2` is defined in
        its own method.

        :return: Price calculated for the Binary Put option.
        """
        d2 = self.get_d2()
        price = exp(-self.r * self.t) * (1 - norm.cdf(d2))
        return price
    ...
    def get_delta(self) -> float:
        ...
        The delta of a Binary option represents the expected change in the price of the option as a response to
        small changes in the price of the underlying (:math:`\\small \\displaystyle \\frac{\\partial V}{\\partial S}`).
        The delta of the Binary Put option is calculated as:
        ...
        :return: Delta calculated for the Binary Put option.
        """
        d2 = self.get_d2()
        delta = - (exp(-self.r * self.t) * norm.pdf(d2)) / (self.sigma * self.s * sqrt(self.t))
        return delta
    ...
    def get_theta(self, periods: int = 1) -> float:
        """
        The theta of a Binary option represents the expected change in the price of the option as a response to
        time passing (:math:`\\small \\displaystyle \\frac{\\partial V}{\\partial t}`). The theta of the Binary Put
        option is calculated as:
        ...
        :param periods: period size to express theta, 365 for calendar days, 252 for trading days, etc.
        :return: Theta calculated for the Binary Put option.
        """
        d1 = self.get_d1()
        d2 = self.get_d2()
